...
MixedScore material(Board board, int& eg)
{
...
MixedScore piece(const Board& board, Bitboard occupancy, Bitboard filter, Bitboard safe_squares_from_pawns, Bitboard our_pawn_attacks)
{
	...
	while (b)
	{
		...
		//int safe_squares = 0;
		//while (attacks)
		//	if (!board.attackers<~TURN>(attacks.bitscan_forward_reset(), occupancy))
		//		safe_squares++;
...
MixedScore king_safety(const Board& board)
{
	...
	constexpr MixedScore pawn_shelter[10] = { MixedScore(-100, 0), MixedScore(-25, 0), MixedScore(0, 0),
											  ...
											  MixedScore(40, -10),  MixedScore(41, -15), MixedScore(42, -20), MixedScore(43, -25) };
	...
	//if (Bitboards::ranks_files[white_king.bitscan_forward()] & black_rooks)
	//	x_rays -= MixedScore(-20, -50);
	//if (Bitboards::ranks_files[black_king.bitscan_forward()] & white_rooks)
	//	x_rays += MixedScore(-20, -50);
	//if (Bitboards::diagonals[white_king.bitscan_forward()] & black_bishops)
	//	x_rays -= MixedScore(-20, -50);
	//if (Bitboards::diagonals[black_king.bitscan_forward()] & white_bishops)
	//	x_rays += MixedScore(-20, -50);
...
Score evaluation(const Position& position, bool output)
{
	...
	//result += turn_to_color(board.turn()) * 15;
